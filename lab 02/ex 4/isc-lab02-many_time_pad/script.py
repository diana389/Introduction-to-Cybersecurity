...
def main():
    ...
    # TODO
    # You know the key starts with "ALEXCTF{"
    partial_key = "ALEXCTF{"

    # Calculate the length of the known partial key
    partial_key_length = len(partial_key)

    # Find the full key by XORing the known partial key with the first ciphertext
    full_key = ''.join([chr(c[0][i] ^ ord(partial_key[i])) for i in range(partial_key_length)])

    decrypt_part(c, full_key)
    # reveal_key(c[0], full_key)
